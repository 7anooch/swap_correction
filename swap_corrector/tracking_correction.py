...
from . import utils, metrics, plotting
...
def compare_filtered_trajectories(main_path: str, output_path: str, file_name: str) -> None:
    """
    Compare raw and filtered trajectories.
    
    Args:
        main_path: Path to the raw data
        output_path: Path to save the output
        file_name: Name of the data file
    """
    # Load data
    raw_data = pd.read_csv(os.path.join(main_path, file_name))
    processed_data = pd.read_csv(os.path.join(main_path, f"{file_name.split('.')[0]}_processed.csv"))
    
    # Generate comparison plot
    fig = plotting.generate_comparison_plot(raw_data, processed_data, fps=30)
    
    # Save plot
    output_file = os.path.join(output_path, f"{file_name.split('.')[0]}_trajectory_comparison.png")
    fig.savefig(output_file)
    plt.close(fig)

def compare_filtered_distributions(main_path: str, output_path: str, file_name: str) -> None:
    """
    Compare distributions from raw and filtered data.
    ...
    fig_raw = plotting.generate_distribution_plot(raw_data, fps=30)
    fig_processed = plotting.generate_distribution_plot(processed_data, fps=30)
    
    # Save plots
    base_name = file_name.split('.')[0]
    fig_raw.savefig(os.path.join(output_path, f"{base_name}_raw_distribution.png"))
    fig_processed.savefig(os.path.join(output_path, f"{base_name}_processed_distribution.png"))
    plt.close(fig_raw)
    plt.close(fig_processed)

def examine_flags(main_path: str, output_path: str, file_name: str) -> None:
    """
    Examine flags in the data.
    ...
    """
    # Load data
    data = pd.read_csv(os.path.join(main_path, file_name))
    ...
    # Save plot
    output_file = os.path.join(output_path, f"{file_name.split('.')[0]}_flags.png")
    fig.savefig(output_file)
    plt.close(fig)

def main(source_dir: str, output_dir: str, config: 'SwapCorrectionConfig') -> None:
    """
    Main function for running the swap correction pipeline.
    
    Args:
        source_dir: Directory containing the raw data
    ...
    for file_name in csv_files:
        ...
        # Load data
        data = pd.read_csv(os.path.join(source_dir, file_name))
        
        # Process data
        processed_data = tracking_correction(
            data,
            fps=30,
        ...
        # Save processed data
        base_name = file_name.split('.')[0]
        processed_data.to_csv(os.path.join(output_dir, f"{base_name}_processed.csv"), index=False)
        
        if config.diagnostic_plots:
            # Generate diagnostic plots
            compare_filtered_trajectories(source_dir, output_dir, file_name)
            compare_filtered_distributions(source_dir, output_dir, file_name)
            examine_flags(source_dir, output_dir, file_name)
