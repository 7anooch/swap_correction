"""
Machine learning feature extraction for swap detection - Original Version.

DEPRECATED: This is a legacy version kept for reference only.
The current default implementation is features_v4 (~36 features).

This was the original 56-feature baseline set used in initial ML experiments.
Includes all position, distance, velocity, angular, geometric, and temporal context features.
...
def extract_all_frame_features_optimized(trial_data: pd.DataFrame, fps: int = 30,
    ...
    """
    Optimized version: Extract features for all frames in a trial.
    ...
    for i in range(n_frames):
        ...
        # Position features (fast array indexing)
        features['head_x'] = xhead[i] if not np.isnan(xhead[i]) else np.nan
        features['head_y'] = yhead[i] if not np.isnan(yhead[i]) else np.nan
        features['tail_x'] = xtail[i] if not np.isnan(xtail[i]) else np.nan
        features['tail_y'] = ytail[i] if not np.isnan(ytail[i]) else np.nan
        features['mid_x'] = xmid[i] if not np.isnan(xmid[i]) else np.nan
        features['mid_y'] = ymid[i] if not np.isnan(ymid[i]) else np.nan
        features['centroid_x'] = xctr[i] if not np.isnan(xctr[i]) else np.nan
        features['centroid_y'] = yctr[i] if not np.isnan(yctr[i]) else np.nan
        ...
        features['head_velocity_magnitude'] = np.sqrt(head_velocity_x[i]**2 + head_velocity_y[i]**2) if not (np.isnan(head_velocity_x[i]) or np.isnan(head_velocity_y[i])) else np.nan
        ...
        features['tail_velocity_magnitude'] = np.sqrt(tail_velocity_x[i]**2 + tail_velocity_y[i]**2) if not (np.isnan(tail_velocity_x[i]) or np.isnan(tail_velocity_y[i])) else np.nan
